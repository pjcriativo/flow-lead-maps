...
await page.getByRole("button", { name: "Testar" }).first().waitFor({ timeout: 60000 });
await page.getByText("Uso no ciclo:").first().waitFor();
await page.getByText("Disponível até o bloqueio:").first().waitFor();
await page.getByText("Créditos incluídos:").first().waitFor();
await page.getByText("Saldo ainda incluído:").first().waitFor();
