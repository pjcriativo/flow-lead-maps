// PROVA DE UI EM PRODUÇÃO — fotografa a tela Pagamentos (estado honesto).
...
const OUT = process.argv[2] ?? join(tmpdir(), "flow-prova-pagamentos-prints");
...
await page.getByText("Pagamentos").first().waitFor({ timeout: 60000 });
await page.getByText("Pagamentos").first().click();
await page.getByText("O que já existe").first().waitFor();
await page.screenshot({ path: join(OUT, "01-admin-pagamentos.png"), fullPage: true });
