...
async function sessao(email) {
  ...
  if (error || !lk)
    throw new Error(`Não foi possível gerar sessão para ${email}: ${error?.message}`);
...
if (testPermRes.status === 200) {
  ok(
    `lead_business_identity executável como authenticated (HTTP 200, resultado: ${permBody.trim()})`,
  );
} else if (permBody.includes("permission denied")) {
  erro(
    "lead_business_identity SEM permissão para authenticated",
    `HTTP ${testPermRes.status}: ${permBody.slice(0, 150)}`,
  );
} else {
...
if (testPermRes2.status === 200) {
  ...
  erro(
    "normalize_lead_identity_part sem permissão",
    `HTTP ${testPermRes2.status}: ${permBody2.slice(0, 80)}`,
  );
}
...
const userOsm = (await admin.auth.admin.listUsers({ page: 1, perPage: 50 })).data?.users?.find(
  (u) => u.email === USUARIOS[0].email,
);
...
if (deltaDB1 >= 0) {
  ok(
    `OSM: banco confirma ${deltaDB1} novos leads (leads antes=${countAntes1}, depois=${countDepois1})`,
  );
} else {
...
if (deletedApify1 && deletedApify1 > 0)
  console.log(
    `  🧹 Limpeza pré-teste: ${deletedApify1} lead(s) de "${NICHO_APIFY}" em ${CIDADE_APIFY} removido(s) da org`,
  );
...
if (deltaDB2 > 0) {
  ok(
    `Apify1: banco confirma ${deltaDB2} novos leads gravados (leads antes=${countAntes2}, depois=${countDepois2})`,
  );
} else {
...
const userApify2 = (await admin.auth.admin.listUsers({ page: 1, perPage: 50 })).data?.users?.find(
  (u) => u.email === USUARIOS[1].email,
);
...
if (res3.done) {
  ok(
    `Apify2: busca concluída (${inseridos3} leads inseridos — ${deltaDB3 === 0 ? "todos já existiam na conta (deduplicação correta)" : deltaDB3 + " novos"})`,
  );
} else {
...
if (logApify2 && logApify2.length > 0 && logApify2[0].quantity >= 0) {
  ok(
    `Apify2: run registrado com ${logApify2[0].quantity} lugares coletados — custo real confirmado`,
  );
} else {
...
if (ativas.length >= 1) {
  ok(
    `Pool: ${ativas.length} chave(s) ativa(s) disponível(is): ${ativas.map((c) => c.apelido).join(", ")}`,
  );
} else {
  erro(
    "Pool: NENHUMA chave ativa",
    JSON.stringify(pool.chaves?.map((c) => `${c.apelido}:${c.status}`)),
  );
}
if (esgotadas.length > 0) {
  console.log(
    `  ⚠️  Atenção: ${esgotadas.length} chave(s) fora do rodízio: ${esgotadas.map((c) => `${c.apelido}(${c.status})`).join(", ")}`,
  );
}
...
if (logsRecentes && logsRecentes.length > 0) {
  ...
  for (const l of logsRecentes) {
    console.log(
      `  • ${l.metadata?.nicho} em ${l.metadata?.cidade} | ${l.quantity} lugares | US$ ${Number(l.cost_usd ?? 0).toFixed(4)} | chave: ${l.metadata?.key_label ?? "?"}`,
    );
  }
} else {
  ok(
    "Sem novos registros de consumo — todas as buscas Apify vieram do cache (correto, economiza crédito)",
  );
}
...
await admin
  ...
  .ilike("query_key", `%${NICHO_APIFY.toLowerCase().replace(/\s+/g, "%")}%`);
console.log(
  `  🧹 ${(cleanLeads ?? 0) + (cleanOsm ?? 0)} lead(s) de teste removido(s), cache apify limpo`,
);
