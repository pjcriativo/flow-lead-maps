// Screenshot da tela PLANOS (padrão premium) em produção — só navega e fotografa.
...
const OUT = process.argv[2] ?? join(tmpdir(), "flow-prova-planos-prints");
...
await page.getByText("Planos").first().waitFor({ timeout: 60000 });
await page.getByText("Planos").first().click();
await page.getByPlaceholder("Buscar pelo nome do plano…").waitFor({ timeout: 20000 });
// espera os DADOS (o painel carrega async) — o plano "Starter" existe em produção
await page.getByText("Starter").first().waitFor({ timeout: 30000 });
await page.waitForTimeout(500);
await page.screenshot({ path: join(OUT, "01-planos-premium.png"), fullPage: true });
