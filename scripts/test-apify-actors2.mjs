...
async function run() {
  ...
  const actors = ["compass~crawler-google-places", "drobnikj~crawler-google-places"];
  for (const actor of actors) {
    const res = await fetch(`https://api.apify.com/v2/acts/${actor}/runs?token=${token}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ searchStringsArray: ["test"], maxCrawledPlacesPerSearch: 1 })
    });
    const json = await res.json();
    console.log(`Actor ${actor} RUN: status=${res.status}, msg=${JSON.stringify(json.error) || "OK"}`);
  }
}
run();
