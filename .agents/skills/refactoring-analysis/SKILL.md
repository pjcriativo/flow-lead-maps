...
Audit a codebase against Martin Fowler's _Refactoring_ (2nd ed.) and produce a prioritized
...
_Done when:_ target, language, and paradigm are recorded.
...
change. Flag an ambiguous case as _potential_ with the context that might justify it.
_Done when:_ every category above has been scanned, not only the ones that fired.
...
finding. _Done when:_ each finding carries a named technique.
...
cohesion (mixed responsibilities → Extract Class / Split Phase). _Done when:_ the dependency
...
heuristics, and DDD checks — read it before this step. _Done when:_ each principle has a
...
is unsafe and name the critical paths to cover first. _Done when:_ the saved report matches the
...
passes. _Done when:_ the checklist is fully satisfied.
