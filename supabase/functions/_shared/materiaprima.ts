...
function extrairTextos(html: string): string {
  // pega texto de títulos/parágrafos/itens (conteúdo com significado)
  const trechos: string[] = [];
  for (const m of html.matchAll(/<(h1|h2|h3|p|li)[^>]*>([\s\S]*?)<\/\1>/gi)) {
    const t = m[2].replace(/<[^>]+>/g, " ").replace(/&[a-z#0-9]+;/gi, " ").replace(/\s+/g, " ").trim();
    if (t.length >= 3 && t.length <= 400) trechos.push(t);
    if (trechos.join(" ").length > 3000) break;
  }
