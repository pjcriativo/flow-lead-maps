...
export async function lerConfigPlataforma(client: Client): Promise<ConfigPlataforma> {
  try {
    const { data } = await client
      ...
      .select(
        "teto_rodada_usd, teto_mes_usd, teto_redes_rodada_usd, teto_redes_mes_usd, dias_validade_site, remetente_nome_padrao, remetente_email_padrao, intervalo_disparo_min_seg, intervalo_disparo_max_seg, max_leads_busca, modelo_ia, modelo_openai",
      )
      .eq("id", true)
      .maybeSingle();
