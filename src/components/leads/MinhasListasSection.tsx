...
import {
  Loader2, RefreshCw, FolderOpen, ArrowLeft, Pencil, Trash2, Download, Wand2,
} from "lucide-react";
...
import {
  listarListas, fetchLeadsDaLista, renomearLista, excluirLista,
...
export function MinhasListasSection({ onOpenRedesign }: { onOpenRedesign?: (leadId: string) => void } = {}) {
  ...
  const handleExcluir = async (l: LeadListComStats) => {
    if (!confirm(`Excluir a lista "${l.name}"? Os ${l.leads_atuais} leads dela também serão removidos. Esta ação não pode ser desfeita.`)) return;
    const prev = listas;
  ...
  if (aberta) {
    return <ListaAberta lista={aberta} onVoltar={() => { setAberta(null); carregar(); }} onOpenRedesign={onOpenRedesign} />;
  }
    ...
    <div className="mx-auto flex w-full max-w-[1400px] flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Cada busca salva vira uma lista. {listas.length} listas.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-border bg-card p-16 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Faça uma busca na aba Buscar — ela vira uma lista salva automaticamente.</p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {listas.map((l) => (
            <div key={l.id} className="flex flex-col gap-3 rounded-xl border border-border bg-card p-4 shadow-[var(--shadow-card)]">
              <button className="flex-1 text-left" onClick={() => setAberta(l)}>
                ...
                <div className="mt-1 text-xs text-muted-foreground">
                  {l.niche}{l.city ? ` · ${l.city}` : ""}{l.uf ? `/${l.uf}` : ""}{l.fonte ? ` · ${l.fonte}` : ""}
                </div>
              ...
              <div className="flex items-center gap-1.5 border-t border-border pt-2">
                ...
                <Button size="sm" variant="ghost" title="Renomear" onClick={() => handleRenomear(l)}><Pencil className="h-4 w-4" /></Button>
                <Button size="sm" variant="ghost" title="Excluir" onClick={() => handleExcluir(l)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
              </div>
            </div>
          ))}
        </div>
...
function ListaAberta({
  ...
  useEffect(() => {
    ...
    fetchLeadsDaLista(lista.id)
      .then((ls) => { if (vivo) setLeads(ls); })
      .catch((e) => { if (vivo) setError(e instanceof Error ? e.message : "Falha ao carregar leads"); })
      .finally(() => { if (vivo) setLoading(false); });
    return () => { vivo = false; };
  }, [lista.id]);

  const ordenados = useMemo(() => [...leads].sort((a, b) => b.score - a.score), [leads]);
  const paginados = paginar(ordenados, Math.min(pagina, Math.max(1, Math.ceil(ordenados.length / PAGE_SIZE))));
  ...
  const exportCsv = () => {
    ...
    const rows = ordenados.map((l) => [
      ...
      l.email, l.website, l.rating, l.review_count, l.status,
    ]);
    ...
    <div className="mx-auto flex w-full max-w-[1400px] flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <Button variant="outline" size="sm" onClick={onVoltar}><ArrowLeft className="h-4 w-4" /> Listas</Button>
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">{lista.name}</h1>
            <p className="text-sm text-muted-foreground">
              {lista.niche}{lista.city ? ` · ${lista.city}` : ""}{lista.uf ? `/${lista.uf}` : ""} · {ordenados.length} leads
            </p>
          </div>
        </div>
        <Button variant="outline" size="sm" onClick={exportCsv} disabled={!leads.length}><Download className="h-4 w-4" /> Exportar CSV</Button>
      </div>
      ...
      {loading ? (
        ...
        <div className="overflow-hidden rounded-xl border border-border bg-card shadow-[var(--shadow-card)]">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5">
                        {onOpenRedesign && l.website && (
                          <Button size="sm" variant="ghost" title="Redesenhar site" className="text-primary" onClick={() => onOpenRedesign(l.id)}>
          ...
          {ordenados.length > PAGE_SIZE && (
            <Paginacao total={ordenados.length} page={Math.min(pagina, Math.max(1, Math.ceil(ordenados.length / PAGE_SIZE)))} onPage={setPagina} />
          )}
        </div>
      )}
    </div>
